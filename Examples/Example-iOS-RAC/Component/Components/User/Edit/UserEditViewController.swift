...
public protocol UserEditPresenterProtocol {
    var viewModel: Property<UserEditViewModel> { get }
    var command: Signal<UserEditViewModel.Command, Never> { get }
}
...
public final class UserEditViewController: UIViewController {
    ...
    public override func viewDidLoad() {
        ...
        presenter.command
            ...
            .observeValues { [weak self] command in
                switch command {
                case .editCompleted:
        ...
        loadingViewController.reactive.presented(from: self) <~ presenter.viewModel.map(\.loginProgress.isLoading)
    }
}
